...
class DefaultController extends Controller
{
    ...
    public function indexAction()
    {
        ...
        for($i=0;$i<count($art);$i++){
            ...
            if($art1[$i] && $art1[$i]->getNewPrix() !=0 && ( $art1[$i]->getNewPrix()< $art1[$i]->getPrix())){
                $article[$i]["prix"] = $art1[$i]->getNewPrix();
            }else{
                $article[$i]["prix"] =  $art[$i]->getPrix();
            }
        ...
        if (array_key_exists($idArticle."-".$colorId."-".$taille."-".$valeur, $panier)) {
            ...
            if($quantite < 1){
                ...
                if($quantite>$Tl->getQteStock()){
                    ...
                    $repository = $em->getRepository('backOfficeBundle:produitsite');
                    $prod = $repository->find($idArticle);
    ...
    public function panierAction()
    {
        ...
        foreach ($panier as $p){
            $em = $this->getDoctrine()->getManager();
            $repository = $em->getRepository('backOfficeBundle:produitsite');
            $art = $repository->findOneById($p->getIdProduitsite());
            if($art)
            {
                $article[$i]["obj"] = $art;
            ...
            {
                if (($key = array_search($p, $panier)) !== false)
                {
                    unset($panier[$key]);
                }
            }
    ...
    public function monDetailCommandeAction($commandeNum,Request $request,$id)
    {
        ...
        foreach ($panier as $p){
            ...
            else
            {
                ...
                $article[$i]['prix']  = $p->getPrix();

                $article[$i]["color"] = $p->getCode();
                $article[$i]["idColor"] = $p->getIdColor();
                $article[$i]["taille"]=$p->getTitleTaille();
                $article[$i]["valeur"]=$p->getValeurTaille();
                $i++;
            }
